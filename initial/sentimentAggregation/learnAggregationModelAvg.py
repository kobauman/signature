...
def learnAggregationModelsCV(trainReviews, featureAvgSent, busImportantFeatures, path):
    ...
    for j, review in enumerate(trainReviews):
        ...
        for aspect in reviewFeatures:
            if review['business_id'] in busImportantFeatures and aspect in busImportantFeatures[review['business_id']]['sentiment'] and busImportantFeatures[review['business_id']]['sentiment'][aspect][1] > 5:
...
def learnAggreationModel(path, limit = np.Inf):
    ...
    #run function
    model = learnAggregationModelsCV(trainReviews, featureAvgSent, busImportantFeatures, path)
    
    #save model
    pickle.dump(model,open(path+'aggregationAvg_%d.model'%counter,'wb'))
