...
def importantFeatureIdentification(infileName, outBusinesFile, outUserFile, 
                                   busInfo = False, userInfo = False, limit = 100000000):
    logger = logging.getLogger('signature.importantFeatureIdentification')
...
#    '''
#    prepare set of important features for businesses
#    '''
#    for busID in busImportantFeatures:
#        busImportantFeatures[busID]['importantFeatures'] = []
#        for feature in busImportantFeatures[busID]['critical']:
#            if len(busImportantFeatures[busID]['critical']['1']) > 3:
#                busImportantFeatures[busID]['importantFeatures'].append(feature)
#            
            
    out_file = open(outBusinesFile,"w")
    out_file.write(json.dumps(busImportantFeatures))
    out_file.close()
...
#        out_file.close()
    ...
    out_file.write(json.dumps(userImportantFeatures))
    out_file.close()
