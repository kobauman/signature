...
def sampler(path, probs = [0.4, 0.8], busThres = 0, userThres = 0):
    logger = logging.getLogger('signature.sampler')
    logger.info('starting sampling')
    
    
    reviews_filename = path +'/yelp_reviews_features.json'
    #load reviews
    ...
    stat_filename = reviews_filename.replace('.json','_stat.json')
    extrain_filename = reviews_filename.replace('.json','_extrain.json')
